...
genai.configure(api_key=st.secrets["GEMINI_API_KEY"])  # Store your API key in Streamlit secrets
...
def generate_recipe_suggestion(ingredients):
    """Generate a recipe using Gemini based on selected ingredients"""
    model = genai.GenerativeModel('gemini-1.5-pro')
    prompt = f"Generate a recipe using these ingredients: {', '.join(ingredients)}. Include dish name, ingredients, instructions, nutritional information, and preparation time."
    response = model.generate_content(prompt)
    try:
        recipe_text = response.text
        # Basic parsing (improve this for better results)
        recipe_data = {
            "name": recipe_text.split("Name: ")[1].split("\n")[0] if "Name: " in recipe_text else "AI Generated Recipe",
            "ingredients": ingredients,
            "instructions": recipe_text.split("Instructions: ")[1].split("Nutritional")[0].strip().split("\n"),
            "nutrition": {"calories": "N/A", "protein": "N/A", "carbs": "N/A"},
            "preparation_time": "N/A"
        }
        if "Calories: " in recipe_text:
            recipe_data["nutrition"]["calories"] = recipe_text.split("Calories: ")[1].split("\n")[0]
        if "Protein: " in recipe_text:
            recipe_data["nutrition"]["protein"] = recipe_text.split("Protein: ")[1].split("\n")[0]
        if "Carbs: " in recipe_text:
            recipe_data["nutrition"]["carbs"] = recipe_text.split("Carbs: ")[1].split("\n")[0]
        if "Preparation Time: " in recipe_text:
            recipe_data["preparation_time"] = recipe_text.split("Preparation Time: ")[1].split("\n")[0]
        return recipe_data
    except Exception as e:
        st.error(f"Error generating recipe: {e}")
        return {"name": "Error Generating Recipe", "ingredients": [], "instructions": [], "nutrition": {}, "preparation_time": ""}

def generate_event_suggestion(guest_count, theme):
    """Generate event planning suggestions using Gemini"""
    model = genai.GenerativeModel('gemini-1.5-pro')
    prompt = f"Generate event planning suggestions for a {theme} themed event with {guest_count} guests. Include theme details, menu suggestions, estimated cost per person, and recommended planning timeline."
    response = model.generate_content(prompt)
    try:
        event_text = response.text
        event_data = {
            "theme_details": event_text.split("Theme Details: ")[1].split("Menu")[0].strip() if "Theme Details: " in event_text else "AI Generated Event",
            "menu": event_text.split("Menu Suggestions: ")[1].split("Estimated")[0].strip().split("\n"),
            "estimated_cost": event_text.split("Estimated Cost: ")[1].split("Planning")[0].strip() if "Estimated Cost: " in event_text else "N/A",
            "preparation_time": event_text.split("Planning Timeline: ")[1].strip() if "Planning Timeline: " in event_text else "N/A"
        }
        return event_data
    except Exception as e:
        st.error(f"Error generating event suggestions: {e}")
        return {"theme_details": "Error Generating Event", "menu": [], "estimated_cost": "N/A", "preparation_time": "N/A"}
...
# Menu Personalization view
    ...
    if st.button("Get Recommendations"):
        with st.spinner("Analyzing your preferences..."):
            ...
            # Additional recommendations based on health goals
            if health_goals:
                st.write("**Additional Recommendations:**")
                for goal in health_goals:
                    st.write(f"- {goal} - {', '.join(menu_options[preferences[0]])}")       
